...
from torch_batch_cg import BatchedCGSolver
...
def main():
    ...
    # TODO: The grad check can not recognize the batch dimension, check one dimension only
    BATCH_SIZE = 1
    cg_solver = BatchedCGSolver(robot_builder=robot_builder, batch=BATCH_SIZE)
    input_b = torch.rand(BATCH_SIZE, cg_solver.ms_solver.NV, 3, dtype=torch.float64, requires_grad=True)
    print(input_b.shape)    
